...
## 2026-08-24: Person-field normalization (Гражданин РФ + падеж)

**Контекст:** Прогон 9 файлов на `qwen3.6:35b-a3b` показал два дефекта полей `debtor/cedent/cessionary`: 1) префикс `Гражданин РФ`/`РФ` попадает в `cessionary` (`"РФ Рябов Александр Владимирович"` вместо `"Рябов..."` — `doc_027` `Договор УПТ 05 08 2024 УПТ-2`), 2) дательный падеж должника (`"Лузиной Екатерине Александровне"` вместо `"Лузина..."` — `doc_025` `Договор УПТЮ 02 07 2025`). Причина — VLM копирует формулировку из текста `с Лузиной...`/`Гражданин РФ ...` как есть, hint не требовал именительный.

**Решение:** Двойная защита: 
1) Промпт: `config/prompts/templates/one-pass.md:35` + `types/upt_rights.md:14`/`upt_costs.md:14`/`upt_notify.md:14` + `config/doc_types/upt_*json:57` `extractionHint` теперь требуют `именительный падеж, без префиксов Гражданин РФ/Гражданка РФ/РФ/ИП` с примерами (`"Лузиной..." -> "Лузина..."`, `"Гражданин РФ Рябов..." -> "Рябов..."`).
2) Код: `src/components/normalize-fields.js:10` `normalizePersonName()` + `PERSON_FIELDS` + правило `person` в `upt_rights/costs/notify` (`fields/firstPassFields/secondPassFields`) — режет `^(граждан(?:ин|ка)\s+рф|граждан(?:ин|ка)|рф)\s+` (`/iu`), fallback даже если `person` не указан. Проверено ретрай-прогоном 2 файлов 2026-08-24: `Лузина Екатерина Александровна` и `Рябов Александр Владимирович` — `confidence 1`.

**Альтернативы:** Морфологический словарь для склонений (тяжёлый, хрупкий), пост-обработка LLM-validator (дорого).

**Trade-off:** Дательный→именительный полностью на совести VLM (prompt); код лишь страхует префиксы. Если VLM всё равно вернёт дательный — останется дательный, но повторный прогон показал исправление.
